...
자기소개입니다. 자기소개입니다.</textarea
  ...
  `},P=()=>{let s=new i().isLogin(),a=m()==="hash"?window.location.hash.slice(1):location.pathname;const e=b();return console.log(`${e}/profile`),a===`${e}/`?h():a===`${e}/profile`?s?$():f():a===`${e}/login`?s?h():f():E()},O=document.getElementById("root"),p=()=>{O.innerHTML=P()},x=document.getElementById("root");p();window.addEventListener("popstate",()=>p());window.addEventListener("hashchange",()=>p());x.addEventListener("submit",t=>y(t));x.addEventListener("click",t=>L(t));
